...
public class BlockAdminChest extends BlockContainer {
	...
	public String getUnlocalizedName() {
		return String.format("tile.%s%s", Reference.MOD_ID.toLowerCase() + ":", BlockInfo.ADM_CHEST_UNLOCALIZED_NAME);	
	}
	...
	public TileEntity createNewTileEntity(World world) {
		return new TileEntityAdmChest();
	}
	
	@Override
	public void breakBlock(World world, int x, int y, int z, int id, int meta) {
		TileEntity te = world.getBlockTileEntity(x, y, z);
		if(te != null && te instanceof TileEntityAdmChest) {
			TileEntityAdmChest admChest = (TileEntityAdmChest)te;
			
			for(int i = 0; i < admChest.getSizeInventory(); i++) {
				ItemStack stack = admChest.getStackInSlotOnClosing(i);
