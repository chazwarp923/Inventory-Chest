...
public class BlockHoppingBuffer extends BlockContainer {
	...
	public BlockHoppingBuffer(int id) {
		super(id, Material.wood);

		setCreativeTab(InvTab.tab);
		setHardness(1.5F);
		setUnlocalizedName(BlockInfo.HOPPING_BUFFER_UNLOCALIZED_NAME);
	...
	public void registerIcons(IconRegister register) {
		topIcon = register.registerIcon(Reference.TEXTURE_LOC + ":"
				+ Textures.INVENTORY_CHEST_TOP);
		sideIcon = register.registerIcon(Reference.TEXTURE_LOC + ":"
				+ Textures.INVENTORY_CHEST_FRONT);
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, InventoryChest.instance, 2,
					world, x, y, z);
		}
		return true;
	}
